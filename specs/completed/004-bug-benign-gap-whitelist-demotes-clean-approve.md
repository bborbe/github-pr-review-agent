...
completed: "2026-09-11T19:13:21Z"
...
- `HasUnverifiedConcerns` (`pkg/verdict.go:286`) therefore has to *guess* the disposition by regex: `not verified|unverified` to detect it, then `benignGapPattern` — a hand-maintained whitelist of "benign explanation" phrasings — to decide whether to believe it.
...
Trace through `HasUnverifiedConcerns` (`pkg/verdict.go:286`):
...
- [ ] `make precommit` exits 0 in the repo root, and CHANGELOG has an entry naming the false-`CHANGES_REQUESTED` fix — evidence: `make precommit` exit 0 AND `grep -ci 'changes_requested' CHANGELOG.md` returns ≥ 1
  - The original evidence scoped the grep to `## Unreleased`, which stops existing the moment the release cuts: after `v0.10.0` was tagged the command reads **0** while the entry sits intact under `## v0.10.0` (verified 2026-09-11 — `sed -n '/## v0.10.0/,/## v0.9/p'` reads 1). Made release-agnostic; the entry is uniquely identifiable by its text, so the wider grep does not weaken the claim.
...
- **Residual risk, accepted and named:** the enum removes the parser's guesswork but cannot stop a model from selecting the wrong value. `discord-assistant#37` would be fixed only insofar as the sharpened prompt (DB 2) leads the model to pick `not-an-issue`. If mislabelling recurs after this ships, the next lever is a host-side cross-check (compare run elapsed time against the soft budget — a `not-verified` on a run that finished well inside the budget is provably a mislabel), **not** another prose pattern. Recorded here so the follow-up is chosen deliberately rather than rediscovered.
  - **2026-09-11 — the lever above was taken.** `348cb3c` (2026-09-01 → v0.6.7) had re-added `mustTierBlockerPattern` + `benignVerificationGapPattern`, violating AC 3 (`grep -c MustCompile pkg/verdict.go` read 3, not 1). Follow-up prompt `023-spec-004-elapsed-keyed-unverified-concerns-gate` (committed in `5b566b7`) deleted both and replaced the prose test with `DemotesUnverifiedConcerns(reviewText, elapsed, budget)` at `pkg/verdict.go:353`, demoting when `elapsed/budget >= 0.8`. At HEAD: AC 1, 2, 3, 4-7 (test suite), 8 and 9 pass. **AC 10 / 11 (Post-Deploy Rung-2 / Rung-3) are unverified** — the fix is committed to local master but not yet pushed, released or deployed, so this spec stays `verifying` until those rungs run.
