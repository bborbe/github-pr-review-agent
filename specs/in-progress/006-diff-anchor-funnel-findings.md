...
- `grep -c '"findings_count":17' pkg/testdata/funnel_fixture_minimal_diff_debt.json` ≥ 1 AND `grep -c '"findings_count":19' pkg/testdata/funnel_fixture_diff_introduces_defect.json` ≥ 1
...
- Fixture sources for the implementation (already built + verified with the real runner on 2026-09-08): the fixture git repo and captured findings JSONs live at `/tmp/fixture-debt` (`metrics.go` with base / feature / feature2 commits; `findings.json` = 17, `findings2.json` = 19). Commit them into `pkg/testdata/` so the unit rows replay from committed data.
...
| 2 | Tests + committed fixtures: hunk-parsing `DescribeTable`, filter `DescribeTable` (both traps), fixture JSONs from `/tmp/fixture-debt` committed into `pkg/testdata/`, fixture rows (17→0, 19→2 at lines 54/55), fail-closed rows, contract row | 2, 6 | 1, 2, 3, 4, 5, 6 | prompt 1 (tests the filter) |
